...
%% Filtrar imagen
xorg=C02tN(:,:,1);%aqui se ajustan la imagenes a observar, si desa una secuancia use un For
x=xorg;
x=imnlmfilt(xorg,'ComparisonWindowSize',3,'SearchWindowSize',15,"DegreeOfSmoothing",0.04);
figure
imshow([xorg x])
...
figure
imshow([x, (yyRL-min(yyRL(:)))/(max(yyRL(:)))-min(yyRL(:))],[],'InitialMagnification',1024)
